...
void Grouped_Endcap_Time(int NStt, int Ntr, float PosInStt[][3], float PosOuStt[][3], int IdStt[], float EdqStt[], int IdParStt[], float TimeStt[], float &Edepgbarrel, float &timegbarrel, int &vistogbarrel, TH2D *istogbarrel, float &xb, float &yb, float &zb, int &index){
	...
	for(int i=0; i<NStt; i++){
		...
		if(IdParStt[i]==Ntr){
			...
			if(R>200 || abs(PosInStt[i][0])>220 || (R<200 && abs(PosInStt[i][0])<169)) continue; //scarto barrel, e catcher e stt 
			new_dato=true;
			//cout<<"Radius "<<R<<endl;
			...
			if(abs(PosCalZ)>200){ cout<<"Error after Ideal2Real R in ENDCAP !!"<<R<<" "<<PosCalX<<" "<<PosCalY<<" "<<PosCalZ<<" PosIn "<<PosInStt[i][0]<<" "<<PosInStt[i][1]<<" "<<PosInStt[i][2]<<endl; exit;} 
		//	 cout<<"Endcap point!! R, x"<<R<<" "<<PosInStt[i][0]<<" PosCal "<<PosCalX<<" "<<PosCalY<<" "<<PosCalZ<<endl;} 


			if(n_cell>0){
				for(j=0; j<n_cell; j++){
					//in cm secondo la scelta di Federico
					if(abs(PosCalX-PosCalCell[j][0])<0.5 &&  abs(PosCalZ-PosCalCell[j][2])<0.5) {
						new_dato=false;
						break;
					}
	...
	for(int i=0; i<NStt; i++){
		...
		if(IdParStt[i]==Ntr){
			...
			if(R>200 || abs(PosInStt[i][0])>220 || (R<200 && abs(PosInStt[i][0])<169)) continue;
			...
			if((TimeWeigCalCell[j]+barreltimecut)>=TimeCal) {   //USO il tempo minimo e non TimeWeigCalCell
	...
	for(int k=0; k<n_cell; k++){
		...
		//if(EnCutCalCell[k]>sogliabarrel && TimeCalCell[k]<timecutcell){  //USO il tempo minimo e non TimeWeigCalCell
		if(EnCutCalCell[k]>sogliabarrel && TimeWeigCalCell[k]<timecutcell) {
			...
			//if(TimeCalCell[k]<timegbarrel) {  //USO il tempo minimo e non TimeWeigCalCell
